...
Driver* insertDriver(string args) {
    InputParser inputParser = InputParser();
    int id, age;
    char status;
    int experience, vehicleId;

    if(inputParser.checkInput(regex("\\d\\d*,\\d*,[MDWS],\\d\\d*,\\d\\d*"), args)) {
        vector<string> temp;
        boost::split(temp, args, boost::is_any_of(","));
...
int main(int argc, char *argv[]) {
    ...
    //create a driver
    Driver* driver = insertDriver(argv[3]);
